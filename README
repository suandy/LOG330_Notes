...
1.	Fonctionnalités
a.	Capacité implémenté par le logiciel
2.	Qualité
3.	Coût
a.	Le prix du projet
4.	Échéance
a.	Facteur de garantie des tâches à accomplir
5.	Personnel
a.	Quantité de personnel, personnel efficace

COQ – prévention - détection – défaillance – préjudice
Coût de la qualité
•	Coût de prévention
o	Pour prévenir l’occurrence d’erreurs dans les diverses étapes durant le processus de développement ou de maintenance.
•	Coût de détection (ou d’évaluation)
o	Le coût de vérification ou d’évaluation d’un produit ou d’une service pendant les différentes étapes du processus de développement
•	Coût de défaillance
o	Interne
...
1.	Éliciter : rassembler les souhaits, attentes et besoins des parties prenantes
2.	Prioriser : débattre de leur importance relative, par exemple selon deux priorités.
3.	Analyser : vérifier la cohérence et l’exhaustivité.
4.	Décrire : écrire les exigences sous une forme aisément compréhensible pour les utilisateurs et les développeurs.
5.	Spécifier : transformer les exigences d’affaires en spécification logicielle
6.	Réviser
...
1.	Perspective transcendantale de la qualité : Quoique la qualité ne puisse pa être définie, vous le savez quand vous l’expérimentez.
2.	Perspective de l’utilisateur : Le logicel de qualité fait ce qu’il doit faire du point de vue de son utilisateur.
3.	Perspective de la fabrication : La qualité est définie comme étant la conformité aux spécifications.
4.	Perspective de la qualité du produit : La perspective interne du produit comme par exemple la qualité de son architecture.
5.	Perspective de la qualité comme valeur : Vise à éliminer toutes les activités qui n’ont pas de valeur ajoutée. Par exemple, la rédaction de certaines documentations.
...
Les processus de soutien décrits dans ITIL sont orientés vers les opérations au jour le jour. Leurs objectifs premiers sont de résoudre les difficultés au moment où elles se produisent ou d’éviter qu’elles se produisent lors d’un changement dans l’environnement informatique ou dans les façons de faire de l’organisme. Décrit les 5 processus suivants :
...
Évaluer la conformité (spécifications)
Trouver les anomalies
Trouver les anomalies et de les améliorer/examiner
Évaluer la conformité (normes)
Leadership
Gestionnaire
Ingénieur en chef
Facilitateur entraîné
Facilitateur ou auteur
Auditeur en chef
Nombre participants
Illimité
Illimité
...
o	Analyse des réponses et revue de la documentation
♣	Visites au site (entrevues et revues)
•	Constats (basés sur le modèle ou la norme utilisée)
o	Représentation graphique du Profil de secteur clé
